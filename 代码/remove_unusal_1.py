...
def create_histogram_all_data(df, column_name, id_column='id'):
    ...
    # Plot histogram for all data with 50 bins
...
def create_zoomed_histogram(df, column_name, range_min, range_max, id_column='id'):
    """
    Create histogram for specific range of character lengths
    """
    # Calculate character lengths
    lengths = df[column_name].astype(str).apply(len)
    lengths = lengths.replace('nan', 0).astype(float)
    ...
    filtered_lengths = lengths[(lengths >= range_min) & (lengths <= range_max)]

    if len(filtered_lengths) == 0:
        print(f"No data found in range [{range_min}, {range_max}] for column {column_name}")
        return

    print(f"\nZoomed Analysis for {column_name} in range [{range_min}, {range_max}]:")
    print(f"  Data points in range: {len(filtered_lengths)}")
    print(f"  Mean length in range: {filtered_lengths.mean():.2f}")
    print(f"  Min length in range: {filtered_lengths.min():.0f}")
    print(f"  Max length in range: {filtered_lengths.max():.0f}")

    # Create figure
    plt.figure(figsize=(12, 8))

    # Plot histogram for filtered data with 50 bins
    if len(filtered_lengths) > 0:
        plt.hist(filtered_lengths, bins=50, alpha=0.7, color='green', edgecolor='black')

    plt.title(
        f'Zoomed Distribution of Character Lengths in {column_name} Column\n(Range: {range_min}-{range_max}, Total in range: {len(filtered_lengths)})',
        fontsize=14)
    plt.xlabel('Character Length', fontsize=12)
    plt.ylabel('Frequency', fontsize=12)
    plt.grid(True, alpha=0.3)
...
def analyze_csv_outliers(file_path, columns_to_check=['desc', 'textExtra.hashtagName'], id_column='id'):
    ...
    for column in columns_to_check:
        if column in df.columns:
            ...
            # Create zoomed histograms for specific ranges based on column name
            if column.lower() == 'desc':
                print(f"\nGenerating zoomed histogram for {column} in range [1000, 1500]...")
                create_zoomed_histogram(df, column, 750, 1000, id_column)
            elif 'hashtag' in column.lower():
                print(f"\nGenerating zoomed histogram for {column} in range [200, 400]...")
                create_zoomed_histogram(df, column, 200, 300, id_column)
